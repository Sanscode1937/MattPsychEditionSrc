...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Please Just Give Up', 0.2], //From 0% to 19%
		['Your Life Is Comepletely Fucked', 0.4], //From 20% to 39%
		['Keep Up Man', 0.5], //From 40% to 49%
		['Oh Ok?', 0.6], //From 50% to 59%
		['Oooooo Geeez', 0.69], //From 60% to 68%
		['Good', 0.7], //69%
		['OOOO That Sick!', 0.8], //From 70% to 79%
		['Sheesh', 0.9], //From 80% to 89%
		['How the fuck how did you do that', 1], //From 90% to 99%
		['Man you are not human', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'Arena':
				var bg:BGSprite = new BGSprite('arena-bg', -600, -200, 0.9, 0.9);
				add(bg);
		
				var idunno:FlxSprite = new FlxSprite(-600, 167);
				idunno.frames = Paths.getSparrowAtlas('arena-characters');
				idunno.animation.addByPrefix('arena-characters', "bg-characters", 30);
				idunno.animation.play('arena-characters');
				idunno.updateHitbox();				
				add(idunno);
			...
			case 'Arena2':
				...
				var bg:BGSprite = new BGSprite('boxingnight3', -600, -200, 0.9, 0.9);
				add(bg);			
				bg.updateHitbox();	
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['wiik1_nomiss', 'wiik2_nomiss', 'week3_nomiss', 'week4_nomiss',
		...
		if (!note.wasGoodHit)
		{
			...
			if(note.hitCausesMiss) {
				...
				switch(note.noteType) {
					...
					case 'Suffering Note': //Suffering Note
						if(boyfriend.animation.getByName('hurt') != null) {
							boyfriend.playAnim('hurt', true);
							boyfriend.specialAnim = true;							
						}
	...
	private function checkForAchievement(achievesToCheck:Array<String>):String {
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName)) {
				...
				{
					case 'wiik1_nomiss' | 'wiik2_nomiss' | 'week3_nomiss' | 'week4_nomiss' | 'week5_nomiss' | 'week6_nomiss' | 'week7_nomiss':
